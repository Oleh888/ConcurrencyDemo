...
public class SumForkJoinCounter extends RecursiveTask<Long> {
    ...
    protected Long compute() {
        if (numbers.size() - 1 > THRESHOLD) {
            return ForkJoinTask.invokeAll(createSubTasks(numbers.size() - 1)).stream().mapToLong(ForkJoinTask::join).sum();
        }
    ...
    private Collection<RecursiveTask<Long>> createSubTasks(int end) {
        ...
        List<Long> firstHalfOfNumbers = LongStream.rangeClosed(numbers.get(0), numbers.get((end) / 2))
                .boxed()
                .collect(Collectors.toList());
        List<Long> secondHalfOfNumbers = LongStream.rangeClosed(numbers.get((end) / 2 + 1), numbers.get(end))
                .boxed()
                .collect(Collectors.toList());
